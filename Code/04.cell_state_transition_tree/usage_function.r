...
outname = "test"
	...
	for(date_idx in 1:(length(order_date)-1)){
		...
		if(length(integration.anchors@object.list) == 2){
			source('~/pcisatlas/scripts/getMaxAnchor.R')
			#lapply(list.files(path = "~/pcisatlas/03.3rd/Seurat.Rscript-4.4.0/01.SeuratR4Maxanchors", pattern = "\\.R$", full.names = TRUE), source)
...
outname = "test"
	...
	system(paste0("cp ~/pcisatlas/02.2nd/11.Fig3/03.organogenesis.cellTraj/test/11.TOME_roadmap/99.test_code_seurat/TOME_script/tome_code/help_code/create_map.py ", create_map_name))
	...
	system(paste("cp ~/pcisatlas/02.2nd/11.Fig3/03.organogenesis.cellTraj/test/11.TOME_roadmap/99.test_code_seurat/TOME_script/map.sxz.html", map_file_name))
